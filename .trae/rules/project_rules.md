...
- 视图组件：用于展示数据或处理用户交互的组件，应放在 views/ 目录下。
- 功能组件：用于实现具体功能的组件，应放在 components/ 目录下， 组件采用 文件名/组件名.vue 格式 使用index.ts 导出组件 及 类型定义。
- 组件样式：优先使用 Tailwind CSS 类名，避免自定义样式。
...
- 路由命名：与组件命名保持一致，使用短横线分隔命名（kebab-case）。
- 样式命名：自定义样式使用短横线分隔命名（kebab-case），避免下划线或驼峰命名。
